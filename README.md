...
10. **[aimax ↔ Spark 100G over OCuLink](10-aimax-spark-100g-oculink-link.md)** — New lane: x86 workstation (Ryzen AI MAX+ 395) to `edgexpert-9105` via ConnectX-5 in an AOOSTAR AG01 OCuLink dock. Link negotiates 100GbE but delivers **22.3 Gb/s** — the ceiling is PCIe 3.0 x4 (31.5 Gb/s raw), imposed by the M.2→OCuLink path, not the wire. Run ended when the NIC took a fatal error under 8-stream load and dropped off the bus (`rev ff`, BARs gone, recoverable only by power cycle). Includes component elimination — QSFP cable and Spark ruled out, thermal/dock-power the leading suspects — and the AER-disabled caveat that limits what the logs can prove. *(superseded by report 11 — the fault was ASPM, not thermal.)*
11. **[OCuLink lane resolved](11-oculink-lane-resolved.md)** — Closes out report 10. The fatal PCIe drop was **ASPM**, fixed by `pcie_aspm=off pci=realloc`; the card now survives the exact 1→4→8 ramp that killed it. Bandwidth rose to **28.0 Gb/s** (89% of the Gen3 x4 ceiling) and RDMA write latency is **1.34 µs** — ~4× better than kyuz0's E810 reference, and the number that makes **tensor parallelism viable**. Thermal and dock slot power both disproved in the same run: NIC temperature stayed flat at 70–71 °C from idle through 20 s of 8-stream load.
